...
There are only two employees in the Sales department, Henry earns the highest salary
...
select Employee.DepartmentId DId, Employee.Name as Employee, Employee.Salary as Salary,
dense_rank() over( partition by Employee.DepartmentId order by Salary desc) SalaryRank
from Employee

) as SalaryRankList
inner join Department on SalaryRankList.DId = Department.Id
where SalaryRank <=3