...
//        data = new int[]{12,97,25,64,17,93,9,52,42,85,81,84,1,54,15,72,11,94};
    ...
    private static void doRun(int expected, int k, int[] prices){
        int res = new N188HBestTimetoBuyandSellStockIV().maxProfit(k, prices);
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        for (int i = 0; i < prices.length; i++) {
            for (int j = 0; j < k; j++){
                int lastBalance = j == 0 ? 0 : balance[j - 1][0];
                balance[j][0] = Math.max(balance[j][0], balance[j][1] + prices[i]); //after sold
                balance[j][1] = Math.max(balance[j][1], lastBalance - prices[i]);   //after buy
            }
        }

        int res = 0;
        for(int[] item : balance) res = Math.max(res, item[0]);
        return res;
    }

    //from solution
    //Runtime: 14 ms, faster than 23.19% of Java online submissions for Best Time to Buy and Sell Stock IV.
    //Memory Usage: 42 MB, less than 70.22% of Java online submissions for Best Time to Buy and Sell Stock IV.
    //merge
    public int maxProfit_1(int k, int[] prices) {
        ...
        int res = 0;
        for (int[] t : transactions) res += prices[t[1]] - prices[t[0]];
        return res;
    }
